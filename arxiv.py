...
def save_and_translate(papers, filename='arxiv.json'):
    ...
    cached_title2idx = {result['url'].lower():i for i, result in enumerate(results)}
    
    untranslated_papers = []
    untranslated_papers_title = []
    translated_papers = []
    ...
    for paper in papers:
        ...
        if title in cached_title2idx.keys():
            ...
            untranslated_papers_title.append({"title": title, "url":url})
    ...
    results.extend(untranslated_papers_title)
